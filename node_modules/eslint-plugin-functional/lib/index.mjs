...
const config$2 = deepMerge(config$1, {
    rules: {
        "functional/functional-parameters": "error"
    }
});

const config$3 = deepMerge(config$1, {
    rules: {
        "functional/no-let": "error",
        "functional/immutable-data": "error"
    },
...
const config$4 = deepMerge(config$1, {
    rules: {
        "functional/no-throw-statement": "error",
        "functional/no-try-statement": "error"
    }
...
const config$5 = deepMerge(config$1, {
    rules: {
        "functional/no-this-expression": "error",
        "functional/no-class": "error"
    },
...
const config$6 = deepMerge(config$1, {
    rules: {
        "functional/no-expression-statement": "error",
        "functional/no-conditional-statement": "error",
        "functional/no-loop-statement": "error"
 ...
 */
function getParentOfType(checker, node, child = null) {
...
function inFunctionBody(node) {
    return (getParentOfType((n, c) => isFunctionLike(n) && n.body === c, node) !== null);
}
/**
...
function isInReturnType(node) {
    return (getParentOfType((n) => n.parent != undefined &&
...
function createRule(name, meta, defaultOptions, ruleFunctionsMap) {
    return ESLintUtils.RuleCreator(name => `https://github.com/jonaskello/eslint-plugin-functional/blob/v${version}/docs/rules/${name}.md`)({
        ...
        create: (context, [options]) => Object.entries(ruleFunctionsMap)
            ...
            .reduce((carry, object) => (Object.assign({}, carry, object)), {})
 ...
 * Check if the given node violates this rule.
 */
function checkUpdateExpression(node, context) {
    return {
        context,
...
const mutableToImmutableTypes = new Map([["Array", "ReadonlyArray"], ["Map", "ReadonlyMap"], ["Set", "ReadonlySet"]]);
/**
